...
if numOfCondition=="2":
    ...
    for row in reader:
        ...
        try: # if statment, when it is not error   
            ...
            condition[2][1]['float num pts']=float(condition[2][1]['num pts'])
            condition[2][2]['float num pts']=float(condition[2][2]['num pts'])
            condition[2][3]['float num pts']=float(condition[2][3]['num pts'])
            condition[2][4]['float num pts']=float(condition[2][4]['num pts'])
            condition[2]['sum pts']=condition[2][1]['float num pts']+condition[2][2]['float num pts']+condition[2][3]['float num pts']+condition[2][4]['float num pts']
            ...
            tempRow.append(condition[2]['sum pts'])#[6]

            condition2_Standard_error=np.sqrt(np.diag(condition[2]['pcov']))
            condition[2]['fit output'].extend(condition2_Standard_error)#[4,5,6,7]=[Berror, Aerror, Chalferror, berror]
            condition2_ConfidenceIntervcondition2_chalf=t.ppf(.975, (condition[2]['sum pts']-1))* condition[2]['fit output'][6]/np.sqrt(condition[2]['sum pts'])
            condition[2]['fit output'].append(condition2_ConfidenceIntervcondition2_chalf)#[8]
            if denatureMethod=="CD":
                CI_ratioTOrange2=condition2_ConfidenceIntervcondition2_chalf/CD_conc_range
                condition[2]['fit output'].append(CI_ratioTOrange2)#[9]
            if denatureMethod=="HD":
                CI_ratioTOrange2=condition2_ConfidenceIntervcondition2_chalf/1
                condition[2]['fit output'].append(CI_ratioTOrange2)#[9]
            condition2_CI_Chalf_lowbound=condition[2]['C half']-condition[2]['fit output'][8]
            condition2_CI_Chalf_upbound=condition[2]['C half']+condition[2]['fit output'][8]
            condition[2]['fit output'].append(condition2_CI_Chalf_lowbound)#[10]
            condition[2]['fit output'].append(condition2_CI_Chalf_upbound)#[11]
            condition2_ConfidenceIntervcondition2_b=t.ppf(.975, (condition[2]['sum pts']-1))* condition[2]['fit output'][7]/np.sqrt(condition[2]['sum pts'])
            condition[2]['fit output'].append(condition2_ConfidenceIntervcondition2_b)#[12]
            condition2_CI_b_lowbound=condition[2]['b']-condition[2]['fit output'][12]
            condition2_CI_b_upbound=condition[2]['b']+condition[2]['fit output'][12]
            condition[2]['fit output'].append(condition2_CI_b_lowbound)#[13]
            condition[2]['fit output'].append(condition2_CI_b_upbound)#[14]
