...
Main code
"""
infile_name, outfile_name, outimg_name = ri.read_masterfile(master_file)
ri.define_headers(infile_name)
condition = ri.condition
print(condition)

finalData = []

infile = open(infile_name, "r") # "r" = reading mode
...
for row in reader:
    ...
    if any(x.strip() for x in row):
        ...
        title = row[ri.rowLoc]
        ID = row[ri.IDLoc]
        proteinName = row[ri.NameLoc]
        Peptide = row[ri.PeptideLoc]
        ...
        try:
            ...
            figure_filename = outimg_name + title + "_" + proteinName
