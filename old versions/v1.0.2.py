...
# if using an older version, you will need to go up another directory in file locations
...
# file selection
infile_locationAndName = "../test_pythonIn.csv"
outfile_locationAndName = "../test_fitOutput.csv"
outImage_location = "../testoutput2//"
        ...
        try: # if statment, when it is not error
            ...
            rep1_fit_result_output.extend(popt1) # extend merge two list to one big list
                                                 # popt has all the parameter (variable) in regression equation (x, B, A, Chalf, b)
                                                                                     #row rep1_fit_result_output[-, 0, 1,     2, 3]
            ...
            # check this section (need CI from SE(?); >4 points should have)
           ## SE = SD / sqrt(n)
           ## is n already a variable? ask Lavender
            rep1_Standard_error = np.sqrt(np.diag(pcov1))
            # combines rep1_fit_result_output + rep1_Standard_error
            rep1_fit_result_output.extend(rep1_Standard_error) # rep1_fit_result_output[4, 5, 6, 7] = [Berror, Aerror, Chalferror, berror]
           ## CI = (mean + (1.96 * SE)) to (mean - (1.96 * SE))
            ...
            rep1_fit_result_output.append(rep1_ConfidenceIntervrep1_chalf) # rep1_fit_result_output[8]
            if denatureMethod == "CD":
                CI_ratioTOrange = rep1_ConfidenceIntervrep1_chalf / CD_conc_range
                rep1_fit_result_output.append(CI_ratioTOrange) # rep1_fit_result_output[9]
            ...
            rep1_fit_result_output.append(rep1_CI_Chalf_lowbound) # rep1_fit_result_output[10]
            rep1_fit_result_output.append(rep1_CI_Chalf_upbound) # rep1_fit_result_output[11]
            rep1_ConfidenceIntervrep1_b = t.ppf(.975, (sumPoints_AL-1)) * (rep1_fit_result_output[7] / np.sqrt(sumPoints_AL))
            rep1_fit_result_output.append(rep1_ConfidenceIntervrep1_b) # rep1_fit_result_output[12]
            rep1_CI_b_lowbound = rep1_b-rep1_fit_result_output[12]
            rep1_CI_b_upbound = rep1_b+rep1_fit_result_output[12]
            rep1_fit_result_output.append(rep1_CI_b_lowbound) # rep1_fit_result_output[13]
            rep1_fit_result_output.append(rep1_CI_b_upbound) # rep1_fit_result_output[14]
